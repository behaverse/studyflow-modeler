...
/**
 * Every shipped example by name: its file less `.studyflow.yaml`. The name is also its gallery card's test id.
 */
const examplePaths = new Map<string, string>();
for (const rel of globSync('*/examples/*.studyflow.yaml', { cwd: SKILLS_DIR })) {
  const name = path.basename(rel).replace(/\.studyflow\.yaml$/, '');
...
/** The example's file; `setInputFiles` opens it as the app would. */
