...
test('the inspector panel stays between a legible minimum and half the window, and leaves the canvas room', () => {
  const CASES: Array<[string, number, number]> = [
    ...
    ['never past the most the window allows', clampPanelWidth(5000, 1440), maxPanelWidth(1440)],
    ...
    ['a narrow window allows half of itself', maxPanelWidth(900), 450],
  ...
  // Whatever the window, the panel never takes more than half of it, never less than the minimum, and a wider
  // window never allows less; on a wide one the cap stops growing, so the canvas keeps room.
  let previous = 0;
  for (let viewport = MIN_PANEL_WIDTH; viewport <= 4000; viewport += 100) {
    const most = maxPanelWidth(viewport);
    expect(most, `${viewport}px`).toBeGreaterThanOrEqual(MIN_PANEL_WIDTH);
    expect(most, `${viewport}px`).toBeLessThanOrEqual(Math.max(MIN_PANEL_WIDTH, viewport / 2));
    expect(most, `${viewport}px`).toBeGreaterThanOrEqual(previous);
    previous = most;
  }
  expect(maxPanelWidth(4000)).toBeLessThan(2000);
});
