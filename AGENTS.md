...
- `packages/` names no skill but the core ones, whose schemas say `optional: false` (`studyflow`, `prov`, `cognitive`, `local`). A skill declares what the apps need, in schema annotations ([skills/SCHEMAS.md](skills/SCHEMAS.md)), `modeler.ts` or `browser/vite.ts`, and the apps find it.
