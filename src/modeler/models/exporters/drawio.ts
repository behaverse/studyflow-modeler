/**
 * draw.io (diagrams.net) exporter maps the diagram on the canvas onto draw.io's own BPMN 2.0 shape library (`mxgraph.bpmn`)
 */

import { readChoreographyBands } from '@/core/codec/choreography';
import { getDiagramName } from '@/modeler/models/diagramName';
import { choreographyBandHeight } from '@/modeler/models/render/choreography';
...
/** `taskMarker` per BPMN activity type; anything else falls back to `abstract`. */
...
/** `symbol` per BPMN event definition. */
...
/**
 * The marker drawn inside each BPMN gateway diamond. draw.io's `gwType` only
 * covers the X, + and * gateways; the inclusive circle and the event-based
 * pentagon are reached through the same `outline`/`symbol` pair its event
 * stencil uses. An unnamed gateway type falls through to a plain diamond.
 */
...
/** Types that frame their contents by nesting: outermost first. */
const CONTAINER_TYPES = new Set(['bpmn:Participant', 'bpmn:Lane']);

/**
 * A choreography task is the one shape draw.io builds out of parts rather than
 * from a single stencil: a stack container holding a participant band, the task
 * body, and a second band. These are the styles its own palette uses.
 */
...
/** Fill of the non-initiating band, matching the canvas renderer's shading. */
...
/** A cell `value` is rendered as HTML (`html=1`), so element text is escaped
 *  once for HTML and once more by `escapeXml` when it is written as an
 *  attribute; newlines become real `<br>` breaks. */
...
/** The label bpmn-js shows for an element */
...
/** Loop/multi-instance/ad-hoc markers  */
...
/** `outline` selects the ring draw.io draws around an event's symbol. */
...
function shapeStyle(element: any, bo: any): string {
  ...
  if (type === 'bpmn:TextAnnotation') {
    // `annotation_1` is the left-hand bracket BPMN draws; `_2` is its mirror.
  ...
  if (type === 'bpmn:SubProcess' || type === 'bpmn:Transaction' || type === 'bpmn:AdHocSubProcess') {
    // `bpmnShapeType` is only for the double-ruled transaction: draw.io reads
    // its `subprocess` value as *event* sub-process and dashes the border.
    const transaction = type === 'bpmn:Transaction' ? 'bpmnShapeType=transaction;' : '';
    // A collapsed sub-process gets the `+` marker; an expanded one is the plain
    // frame its children sit on top of, so its label moves out of their way.
  ...
  // Tasks, choreography tasks (draw.io has no choreography stencil), and any
  // activity type the table above does not name.
...
function edgeStyle(element: any, bo: any): string {
  ...
  if (type === 'bpmn:Association' || type === 'bpmn:DataInputAssociation'
    ...
    // BPMN draws data associations with an arrow head; a plain association only
    // has one when it declares a direction.
...
/** bpmn-js keeps element colors on the DI, under either the `bioc` or the
 *  `color` namespace depending on which tool wrote them. */
...
/**
 * A choreography task, as the frame plus the three stacked parts draw.io builds it from
 */
...
function edgeCell(element: any, known: Set<string>): string {
  ...
  // Endpoints attached to a cell are re-derived by draw.io from the shape
  // perimeter; only a dangling end needs to be pinned to a fixed point.
  ...
  // The first and last waypoints sit on the shape borders; only the bends
  // between them are waypoints in draw.io's sense.
...
/**
 * Render the diagram currently on the canvas as a draw.io
 */
export function exportToDrawio(modeler: any): string {
  ...
  const name = getDiagramName(modeler) ?? 'Studyflow';
