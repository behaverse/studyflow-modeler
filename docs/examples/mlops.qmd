...
4. **Model training** -- a `Script` activity that consumes the training table and produces a model artifact: a data object whose `uri` says where the fitted model is written (the `ml` schema ships a **Model** template for it).
...
8. **Deployment** -- a `Script` activity that writes the model artifact to the production store.
...
- **What was deployed stays on the record.** The model artifact names where its bytes live (`uri`), the study carries a `version`, and the `prov:Activity` trail records which run produced it, with what tool, and when.
...
- [Data](../reference/data.qmd) -- `Dataset`, `Table`, `DataCatalog`.
