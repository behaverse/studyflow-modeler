...
Each visual element in the modeler maps to a class definition in the Studyflow LinkML schemas.

- Event elements map to classes such as `StartEvent` and `EndEvent`, then to moddle entries for BPMN event types.
- Activity elements (e.g., `Instruction`, `Questionnaire`, `CognitiveTask`) map to classes with domain-specific attributes shown in the inspector.
- Gateway elements map to classes like `RandomGateway`, with control-flow attributes encoded in schema properties.
- Data elements map to classes such as `Dataset`, `Table`, and `DataCatalog`.
