...
These elements behave exactly as in BPMN and render with standard BPMN shapes. A BPMN-only tool can open a `.studyflow` file and see a well-formed process -- the Studyflow-specific elements and attributes degrade to BPMN without breaking the diagram.
...
Studyflow does **not** redefine serialization conventions (XML namespaces, element IDs).

This means: any conforming BPMN renderer or analyzer can process a `.studyflow` file and produce something meaningful, even if it ignores Studyflow extensions. The serialized form is BPMN XML with a `studyflow:` namespace for the extensions (i.e., elements inside the `<extensionElements>` blocks). This design choice maximizes compatibility with existing BPMN tools and libraries.
