...
Studyflow extends [BPMN 2.0](https://www.omg.org/spec/BPMN/2.0/) rather than replacing it. If you already know BPMN, this page tells you *what is standard process notation* and *what is domain-specific*.
...
- `CognitiveTask`, `Questionnaire`, `Instruction`, `Rest` (a video-game paradigm is a `CognitiveTask` palette template, not its own type)
- `BehaverseTask` (standardized cognitive test from the [Behaverse](https://behaverse.org) catalog)
...
- `Timeseries` and `EventMarker` -- continuous recordings and the sparse markers over them.
