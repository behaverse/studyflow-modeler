...
- `uses` -- a scheme-prefixed reference, `<scheme>://<ref>[@<version-or-digest>]`. The runner recognizes `python://` (an importable callable, e.g. `python://pkg_for_st.do_map@1.2`), `docker://` (an image, e.g. `docker://ghcr.io/lab/img@sha256:...`), and `https://` (a script URL). Other schemes are allowed but flagged as unresolved -- e.g. a cognitive task imported from jsPsych names its plugin as `jspsych://...`. The ref/version split happens at the last `@`, so docker digests parse correctly.
- `with` -- a YAML mapping of arguments passed to the function (GitHub Actions' `with:` block), a `studyflow:YAMLString` value property. The inspector edits it under the Execution tab and reveals it once `uses` is set.
