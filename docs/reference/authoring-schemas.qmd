...
- `core.moddle.yaml` -- the core Studyflow language: `Study`, `StartEvent`, `EndEvent`, `SequenceFlow`, the abstract `BaseElement`/`Activity`/`Arguments` bases, and the data-infrastructure types (`Dataset`, `DataStorage`, `DataCatalog`, `Schema`, `Array`, `Snapshot`).
- `cognitive.moddle.yaml` -- cognitive and behavioral elements (`CognitiveTask`, `VideoGame`, `Rest`, `Questionnaire`, `Instruction`, `RandomGateway`, `EligibilityGateway`). Split out of `core` so adopters can swap the cognitive layer without touching the core.
- `datatrove.moddle.yaml`, `openbci.moddle.yaml`, `omniprocess.moddle.yaml` -- domain extensions you can enable from the modeler's schema palette.
...
- `implementation` -- a scheme-prefixed reference, `<scheme>://<ref>[@<version-or-digest>]`. The runner recognizes `python://` (an importable callable, e.g. `python://pkg_for_st.do_map@1.2`), `docker://` (an image, e.g. `docker://ghcr.io/lab/img@sha256:...`), `https://` (a script URL), and `file://` (a local script). Other schemes are allowed but flagged as unresolved -- e.g. a cognitive task imported from jsPsych names its plugin as `jspsych://...`. The ref/version split happens at the last `@`, so docker digests parse correctly.
- `arguments` -- a YAML mapping of literal keyword arguments passed to the callable, a `studyflow:YAMLString` value property. Values are literals or `$Element` / `$Element.field` references into wired data; the reserved `args` list covers positional-only calls. Wired inputs are not repeated here -- each data input association binds its element to the callable parameter named on the association (`parameter`, defaulting to the element's name). The inspector edits `arguments` under the Execution tab and reveals it once `implementation` is set.
...
Use the schema prefix to reference types from another schema, e.g. `studyflow:YAMLString`, `studyflow:AssignmentAlgorithmEnum`, `studyflow:Arguments`. The functional schema, for example, refers to `functional:Transform#operationType` when pinning a verb's `operationType` on subtypes like `Map`.
