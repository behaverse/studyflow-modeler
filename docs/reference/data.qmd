---
title: Data
description: Elements and operations for data-centric workflows
order: 6
---
...
<details class="indigo">
  <summary>`DataStore` / `DataStoreReference` <span class="badge float-right">BPMN</span></summary>

  Persistent storage of data that can be accessed across multiple process instances. For example, a database or file system. `DataStoreReference` is the visual element used to point at a `DataStore`. Studyflow adds no storage element of its own -- physical storage stays the native `bpmn:DataStore`/`DataStoreReference` -- and refines the logical side with `Dataset`, which extends `DataStoreReference` and points at the store through its `storage` attribute.
</details>


<details class="indigo">
  <summary>`DataObject` / `DataObjectReference` <span class="badge float-right">BPMN</span></summary>
...
<details class="green">
  ...
  A logical collection (possibly multi-table, multi-modal) registered in a `DataCatalog` and stored in a `bpmn:DataStoreReference` named by its `storage` attribute. Carries an optional `schema`, a `format` (`bdm`, `bids`, `psych-ds`, `kedro`, `undefined`), and format-specific properties (`bdmDataLevel` for BDM; `bidsDataType` for BIDS).
</details>

<details class="green">
  <summary>`Schema` <span class="badge float-right">STUDYFLOW</span></summary>

  A formal description of the structure of a named collection of types. For tabular data, column definitions (column names, data types, units, constraints) -- typically authored with [CSVW](https://www.w3.org/TR/tabular-data-primer/). For non-tabular data, dimensions, data types, and other relevant metadata (e.g. LinkML, JSON Schema). Carries a `format` identifier and an inline `body` (or a URL/path to the schema definition).
</details>
...
<details class="green">
  ...
  Tabular data -- rows and columns -- described by CSVW or LinkML. Unlike a `Dataset`, it commits to that structure rather than staying opaque. Carries a `format` (`csv`, `tsv`, `parquet`, `arrow`, `jsonl`; default `csv`), a `schema` reference, and an optional `rowCount`.
</details>

<details class="green">
  <summary>`Timeseries` <span class="badge float-right">STUDYFLOW</span></summary>

  A continuous physiological or behavioral recording sampled over time (EEG, EMG, eye-tracking gaze stream, accelerometer). Carries `samplingRate`, `channelCount`, `units`, `recordingDuration`, and a `format` (`edf`, `bdf`, `fif`, `set`, `parquet`, `zarr`).
</details>

<details class="green">
  <summary>`EventMarker` <span class="badge float-right">STUDYFLOW</span></summary>

  Sparse time-stamped occurrences within a recording -- trial markers, stimulus onsets, button presses. Carries `eventType`, `eventOnset`, `eventOffset`, and `eventCount`. Its recording-time fields are not the Gantt axis's `onset`/`duration`.
</details>

In summary, `DataCatalog` is a registry of datasets (potentially across multiple stores), `Dataset` is a logical collection, and `Table`, `Timeseries`, and `EventMarker` are the concrete structures a dataset is made of. Physical storage has no studyflow type -- it is BPMN's own `DataStore`, named by `Dataset#storage`.

Also note that, while experimental data is generally assumed to be tabular, `Dataset` supports other data types (i.e., `DataObject` or `Timeseries`, including images, videos, brain imaging, and raw sensor recordings).

## Data operations

Data operations are studyflow-specific markers to describe how data is manipulated as it flows through the process. An operator can be implemented as a usual BPMN task (e.g. script task or service task), and the operator marker serves as a semantic annotation indicating that the task is performing a specific type of data transformation. This is derived, not stored: a step counts as a data operation when it names a data verb (`operationType`) or runs software (`implementation`) without being a participant-facing instrument. The `additionalArguments` mapping comes from the abstract `Arguments` trait, and the operation's inputs and outputs are auto-detected from the data association edges drawn on the diagram.
...
<details class="green">
  ...
  Selects a subset of data based on specified criteria. Used for conditional selection (1 &rarr; subset(1)). The difference between filtering and data-driven gateways in BPMN is that filtering changes the dataset, but gateways change the control flow. They are complementary.
</details>

<details class="green">
  <summary>`reduce` <span class="badge float-right">$f$</span></summary>

  Aggregates data by applying a function that combines multiple input values into a single output value. Used for summarization or joining operations (N &rarr; 1 per group or for the entire dataset).
</details>

<details class="green">
  <summary>`group` <span class="badge float-right">$f$</span></summary>

  Organizes data into groups based on specified attributes. Used for categorization and clustering (1 &rarr; G groups). It changes the data structure to a grouped format.
</details>
...
<details class="green">
  ...
  Similar to map, but flattens the resulting data array into a single output array. Used for one-to-many mappings (1 &rarr; N). Relevant to unnesting in data wrangling libraries.
</details>
...
- `transformTables`: Special case of `transform` that applies a series of transformations to tabular data, such as adding, removing, or modifying columns. The result is one or more new tables based on the specified transformations (1+ tables &rarr; 1+ tables).
