...
- `studyflow.moddle.yaml` -- the core Studyflow language: `Study`, `StartEvent`, `EndEvent`, `SequenceFlow`, the abstract `BaseElement`/`Activity`/`DataOperationActivity` bases, and the data-infrastructure types (`Dataset`, `DataStorage`, `DataCatalog`, `Schema`, `Array`, `Snapshot`).
- `cognitive.moddle.yaml` -- cognitive and behavioral elements (`CognitiveTask`, `VideoGame`, `Rest`, `Questionnaire`, `Instruction`, `RandomGateway`, `StratifiedAllocationGateway`, `EligibilityGateway`). Split out of `studyflow` so adopters can swap the cognitive layer without touching the core.
...
The `description` is shown in the schema palette. The optional `icon` (URL or `iconify <name>`) appears next to the schema name.
...
- `isAttr: true` -- serialized as an XML attribute (most scalar properties).
- `isBody: true` -- serialized as the element's text body. Used for free-form content like YAML `configurations`.
...
- `meta.condition.language: json` + `meta.condition.body: { otherField: value }` -- visibility predicate; the property only appears when other fields match.
