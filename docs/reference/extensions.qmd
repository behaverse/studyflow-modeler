---
title: Extensions
description: Extending the Studyflow with new element types and attributes.
order: 5
---
...
- `isAttr: true` -- serialized as an XML attribute in the BPMN XML form (most scalar properties). The YAML `.studyflow` codec maps every property to a plain key regardless of this flag.
...
## Executable bindings (`uses` / `with`)

Any activity can declare what implements it, GitHub-Actions style. Two properties on the core `studyflow:DataOperationActivity` trait (which mixes onto every `bpmn:Activity`) carry the binding:
...
- `with` -- a YAML mapping of arguments passed to the binding (GitHub Actions' `with:` block), carried as the text body of a `studyflow:With` wrapper. The inspector edits it under the Execution tab and reveals it once `uses` is set.
...
In the BPMN XML form, `uses` is an attribute on the activity (`studyflow:uses="..."`) and `with` is a `<studyflow:with>` child element holding the YAML as its body; both round-trip losslessly through the `.studyflow` codec. An activity that carries a `uses` value shows a function marker on the canvas. The reference runner resolves and validates the binding (the `uses` grammar and that `with` parses as a YAML mapping) but does not execute it in the browser; execution is delegated to a downstream tool (a workflow engine, an evaluation harness, or a container runtime). See `src/assets/examples/bindings_demo.studyflow` for a worked example.
...
## The LinkML companion schema

`src/assets/schemas/studyflow.linkml.yaml` is a hand-maintained LinkML companion to the core moddle schemas. It declares itself the authoritative definition of the core element types and constraints, but it is **not loaded by the modeler** -- the schema loader globs only `*.moddle.yaml` -- and the LinkML exporter generates its output from the diagram's data elements rather than from this file. Keep it in sync manually when changing data element types in `core.moddle.yaml` or `cognitive.moddle.yaml`.
