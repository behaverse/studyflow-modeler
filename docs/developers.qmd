...
A studyflow diagram can be stored as YAML (the canonical format), as BPMN 2.0 XML (which is what other BPMN tools read), or as a PNG or SVG with the diagram embedded in it. Conversion between the three is lossless:
...
There are two runners available. `studyflow run` reads the `runtime` attribute on the study and decides where to send it — a `browser` study runs from the published `./run/` URL, and a `local` one goes to the Python runner on your machine. An AI evaluation is the same walk with a model in the participant's seat (set `agentType` to `bot` on the task), so the same two runners execute it and you do not need a separate harness.
...
: What each runner does with each family of elements, and what is checked before the run starts. A blank means the runner passes that step over. {#tbl-support}
...
These records are also what makes a partial re-run possible: a step is skipped when its record still stands, nothing it reads was re-made during this run, and its outputs are still there. Invalidating a record cascades to everything downstream of the outputs that step re-makes.

## Extending the notation

![The seven element sets that ship with Studyflow: four always load, and three you switch on in the modeler's settings.](assets/img/diagrams/schema-map.svg){#fig-schema-map fig-alt="A flowchart. Four core sets — studyflow, prov, functional, cognitive — always load into the catalog. Three optional sets — ml, agentic, eeg — load only when enabled in Settings, Extensions. The ml set declares no types of its own and roots its twelve presets at the functional set. The catalog compiles every enabled set into the palette, the inspector tabs, and the namespaces the XML may use."}
...
Declaring a type buys a specific and fairly limited amount:
...
To make a new type do something during a run, you have to teach a runner about it, either to render it on screen or to execute it. @tbl-support is the current list of what the two runners handle. The version of a set's format is carried in its namespace.
