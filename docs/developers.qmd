...
![The five element sets that ship with Studyflow: three always load, and two you switch on in the modeler's settings.](assets/img/diagrams/schema-map.svg){#fig-schema-map fig-alt="A flowchart. The three core sets, studyflow, prov, and cognitive, always load into the catalog. The two optional sets, agentic and eeg, load only when enabled in Settings, Extensions. The catalog compiles every enabled set into the palette, the inspector tabs, and the namespaces the XML may use."}
