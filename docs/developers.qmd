...
![The element sets that ship with Studyflow: four always load, and the rest, one per skill, you switch on in the modeler's settings.](assets/img/diagrams/schema-map.svg){#fig-schema-map fig-alt="A flowchart. The four core sets, studyflow, prov, cognitive, and the local runtime, always load into the catalog. The optional sets, behaverse, eeg, agentic, reachy, and the browser runtime, load only when enabled in Settings, Extensions. The catalog compiles every enabled set into the palette, the inspector tabs, and the namespaces the XML may use."}
