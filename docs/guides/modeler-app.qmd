...
Element types ship in layered schemas. The four core schemas -- `studyflow` (the file is `studyflow.moddle.yaml`), `prov`, `functional`, and `cognitive` -- are always loaded and cannot be disabled (Behaverse assessment tasks are part of `cognitive`); the optional extensions -- `agentic`, `ml`, `eeg` -- can be enabled or disabled under *Settings &rarr; Extensions*. Disabled schemas are excluded from the palette and not recognized when opening diagrams (reload the page to apply changes). See [Extensions](../reference/extensions.qmd) for what each extension provides.

## Export formats

Every way a diagram leaves the app is one dialog -- *File &rarr; Export...* -- with the native `.studyflow` file as the default format rather than a separate "save" menu (see `packages/modeler/src/export/`):
