...
`studyflow validate` reports what can be checked without running: whether the file parses, whether the sets it depends on loaded, and any name that no loaded set declares. Warnings are reported but do not fail the command unless you pass `--strict`. Everything else — a condition that can never hold, a data edge that arrives too late — shows up when the study runs.

## Views

A view is a projection: it keeps some of the elements of the diagram and drops the rest. It never adds or renames anything, so a view cannot modify the study it came from.
...
`instrument` names an experiment package (PsychoPy, jsPsych, lab.js, OpenSesame, Behaverse), or a scale, or an identifier of your own, and `configurations` is the YAML passed to that instrument. Setting `agentType` to `bot` has the same instrument answered by a model instead of a person, driven by `botConfigurations`, which is how the same file doubles as an AI evaluation.
...
A pool stands for a group, not for one person. If you draw two pools that deal with each other, the file becomes a collaboration, with one choreography task per exchange — which is how you describe a dyadic experiment, or a study where a person and a model work together.
...
Four operations describe the shape of a data step, each drawn as a service task with a function marker: `Transform` applies a function to data, `Map` applies it to every item in a stream, `Reduce` combines a stream into one value, and `Filter` keeps the items matching a condition. A `Filter` drops *data*, while a gateway changes the *path* a run takes, so the two are not interchangeable.
...
The YAML and the XML convert into each other losslessly, in both directions, so converting a file there and back returns byte-identical text and a version-control diff shows your edits and nothing else. Here is a complete, runnable study:
...
That is [`minimal_study.studyflow.yaml`](assets/examples/minimal_study.studyflow.yaml) in full. Note that there is no geometry in it: the layout follows from `sourceRef` and `targetRef`, and the modeler only saves geometry once you rearrange things by hand.
...
Six short forms collapse the long structure where nothing is lost by doing so, and the long form is always still accepted: a YAML mapping instead of a quoted string under `additionalArguments`, `values`, `configurations`, or a schema `body`; a plain list under `extensionElements`; geometry written on the element it positions; elements keyed by id under `flowElements`; an expression written as a plain string; and `documentation` carrying `checklist` for the checklist entry.
