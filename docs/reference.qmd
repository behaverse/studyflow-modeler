...
Any string attribute may cite a value as `{name}`, or a field of it as `{name.field}`. A name starts with a letter or `_` and holds only letters, digits and `_`, so the braces of YAML or JSON in a value are left alone. Every reader looks the name up in the same order and takes the first value it finds:
