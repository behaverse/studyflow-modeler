...
`format` ties a dataset to a community standard: `bdm` ([Behaverse Data Model](https://behaverse.org/data-model/), from the behaverse skill, whose `behaverse:Dataset` adds `dataLevel`), `bids` ([BIDS](https://bids.neuroimaging.io/), which adds `bidsDataType`), `psych-ds` ([Psych-DS](https://psych-ds.github.io/)), or `nwb` ([NWB](https://www.nwb.org/)).
...
![The files that carry a study, and what converts into what. The thick arrows are the conversions that lose nothing.](assets/img/diagrams/file-format.svg){#fig-round-trip fig-alt="A flowchart. The modeler's diagram saves as .studyflow.yaml and lowers to .bpmn BPMN 2.0 XML, and those two convert into each other losslessly in both directions. The modeler also renders to .studyflow.png and .studyflow.svg, which carry the same BPMN XML beside the pixels as embedded source. An image reopens in the modeler from its embedded source, never from its pixels. A dotted arrow leaves the round trip: skills export one way to .drawio, LinkML, NIDM-Results, and ARTEM-IS."}
