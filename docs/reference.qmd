...
A property may carry an initial `value`; after that only a data edge targeting it changes it. After a run the file holds the values under `state`, keyed by element id. The runner keeps its own entries under `state._meta`: `_meta.prov`, the run records, and `_meta.reached.<element_id>`, how many times a token reached each element, never reset. Names starting with `_` cannot be declared. Any string attribute may cite a value as `{count}` or `{reached}` (resolved from the element outward) or `{state._meta.reached.Excluded_Pre}`, and conditions may test `state._meta.reached.Gate < 4`; the canvas and exported figures show the resolved text, the file and the inspector keep the placeholder.
