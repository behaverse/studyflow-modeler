 ...
 * and a skill is listed by its name. Whether it is required is its schema's to say (`required: true`). */
export type SkillManifest = {
  /** The folder's name, and the runner's (`--runner reachy=…`, `STUDYFLOW_REACHY_PY`). */
  name: string;
  description: string;
  /** `schema`: the BPMN extension the skill contributes (a LinkML `*.moddle.yaml`), relative to its folder. */
  schema?: string;
