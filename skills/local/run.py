...
LOOP_TAGS = {"standardLoopCharacteristics", "multiInstanceLoopCharacteristics"}


# A `{name}` or `{name.field}` citation, per docs/reference.qmd "Placeholders".
PLACEHOLDER = re.compile(r"\{\s*([^\W\d][\w-]*(?:\.[\w-]+)*)\s*\}")
...
class Studyflow:
    ...
    def message_scope(self, element_id: str) -> str:
        """Whose message flows a step exchanges along: its own, else the nearest enclosing sub-process that has some,
        else its pool's. A collapsed sub-process is the only place BPMN can draw a message flow to a step inside it,
        and a pool's flows are the whole pool's, lanes and all, so a step inside one talks along them. A step with
        flows of its own inherits none."""
        scope = element_id
        while scope not in self.flows_in and scope not in self.flows_out:
            container = self.elements.get(self.parents.get(scope) or "")
            if container is None or local(container) not in CONTAINER_TAGS:
                pool = self.participant_of(self.pool_of(element_id)) or ""
                return pool if pool in self.flows_in or pool in self.flows_out else element_id
    ...
    def participant_of(self, process_id: str) -> str | None:
        """The participant whose pool depicts a process, if one does."""
        return next((pid for pid, p in self.participants.items() if p.get("processRef") == process_id), None)

    def instances_of(self, process: ET.Element) -> tuple[str, int]:
        """The pool depicting `process` and how many instances of it run: BPMN's `participantMultiplicity/@maximum`
        (1 with none), which means N instances of that participant's process."""
        pool = self.participants.get(self.participant_of(process.get("id")) or "")
        multiplicity = next((c for c in (pool if pool is not None else ()) if local(c) == "participantMultiplicity"), None)
        maximum = int(multiplicity.get("maximum") or 1) if multiplicity is not None else 1
        return (pool.get("id") if pool is not None else process.get("id")) or "", max(1, maximum)
...
class Runner:
    ...
    def execute_via_runner(self, element: ET.Element, entry: dict, runner: PartialRunner) -> None:
        ...
        talking = flow.message_scope(element_id)
        ...
        self.note_outputs(element, entry)

    def note_outputs(self, element: ET.Element, entry: dict) -> None:
        """What the activity's data output edges targeted, and which of those artifacts are now in the repository.
        A sub-process carries its own edges, so a dataset the steps inside it fill is generated by the sub-process."""
    ...
    def run(self, max_steps: int = 1000) -> None:
        ...
        if len(pools) == 1:
            self.run_pool(pools[0], max_steps)
            return
    ...
    def run_pool(self, pool: ET.Element, max_steps: int) -> None:
        """A pool's process, once, or `participantMultiplicity/@maximum` times when its participant carries one.
        ponytail: the instances run serially, one after another, as a multi-instance activity's passes do; a real
        fan-out would want a thread per instance."""
        participant, instances = self.studyflow.instances_of(pool)
        for instance in range(1, instances + 1):
            if instances > 1:
                # Which instance this is, for whatever runs inside: `state._meta.instance.<pool id>`, 1-based.
                self.state.tree.setdefault("_meta", {}).setdefault("instance", {})[participant] = instance
                self.start_scope(pool.get("id") or "", reset=True)
        ...
        # which the steps inside exchange along, instance by instance (`message_scope`).
        for flow in self.studyflow.flows_out.get(participant, []):
            if flow.get("targetRef") not in self.studyflow.participants:
                self.send(flow, None)

    def walk_pool(self, pool: ET.Element, max_steps: int) -> None:
        try:
            self.run_pool(pool, max_steps)
    ...
    def send(self, flow: ET.Element, content: Any, message_id: str | None = None, in_reply_to: str | None = None) -> None:
        ...
        if target in self.studyflow.participants and runner is not None:
            ...
            return
        if target in self.studyflow.participants and not self.studyflow.participants[target].get("processRef"):
    ...
    def expect_more(self, element_id: str, flows: list[ET.Element]) -> None:
        ...
        def spent(flow: ET.Element) -> bool:
            # A pool no process depicts only answers what is sent to it, and that answer is already here. A pool with
            # a process still has its own message to send until its walk ends — a multi-instance one sends it after
            # its last instance — so a wait on it holds while it runs.
            source = flow.get("sourceRef") or ""
            pool = self.studyflow.pool_of(source)
            return pool in self.pools_done or (pool == source and source in self.studyflow.participants)

        if all(spent(f) for f in flows):
    ...
    def pump(self, element_id: str, talking: str | None = None) -> Any:
        ...
        `talking` is whose flows those are, the element's own or an enclosing sub-process's (`message_scope`)."""
        talking = talking or element_id
        flows_out = {f.get("id"): f for f in self.studyflow.flows_out.get(talking, [])}
        flows_in = [f.get("id") for f in self.studyflow.flows_in.get(talking, [])]
        cache = self.repo_dir / ".cache"
    ...
    def perform(self, element: ET.Element, depth: int, max_steps: int) -> ET.Element | None:
        ...
        try:
            ...
            watching.pop()
        return self.conditional_boundary(element)

    def conditional_boundary(self, element: ET.Element) -> ET.Element | None:
        """The first conditional boundary event of a finished activity whose `condition` holds, with the activity's
        result and bindings already adopted: the walk leaves for it as it does for an error boundary event. The body
        is evaluated like a sequence flow's `conditionExpression` — same evaluator, same `language` rule — and its
        `{placeholders}` read by the usual rule, so `{Play.failedTrialRate} > 0.2` reads the element's own result."""
        element_id = element.get("id") or ""
        for boundary in self.studyflow.boundaries.get(element_id, []):
            condition = next((child for definition in boundary if local(definition) == "conditionalEventDefinition"
                              for child in definition if local(child) == "condition"), None)
            if condition is None or not (condition.text or "").strip():
                continue
    ...
    def loops_again(self, element: ET.Element, passes: int) -> bool:
        ...
        if instances is not None:
            ...
            # marker's semantics is honoured in order of completion only; a fan-out would want a pool per instance.
            cardinality = next((c for c in instances if local(c) == "loopCardinality"), None)
            if cardinality is None or not (cardinality.text or "").strip():
                return passes == 0
            wanted = self.evaluate(cardinality.text.strip(), language=cardinality.get("language"), scope=element.get("id"))
            return passes < int(wanted)
