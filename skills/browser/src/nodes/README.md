...
self-registers via `registerNode(...)`. A definition that needs something fetched before
validation (a build manifest) returns it from `prepare`; it reaches only its own validators.

## How to add a node

1. Create `src/runner/nodes/<newKind>/index.tsx` (the component + registration).
2. Augment `JobsByType` (declared in `@/jobs`) so `Job` includes your kind
